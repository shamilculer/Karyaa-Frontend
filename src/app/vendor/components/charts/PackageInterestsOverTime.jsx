...
import { getPackageInterestsOverTime } from "@/app/actions/vendor/packageAnalytics"

export const description = "Package Interests Over Time Area Chart with Timeframe Filter"
...
const PackageInterestsOverTime = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const result = await getPackageInterestsOverTime(timeframe)
    ...
    const firstPeriodInterests = chartData[0]?.interests || 0;
    const lastPeriodInterests = chartData[chartData.length - 1]?.interests || 0;
    const interestChange = lastPeriodInterests - firstPeriodInterests;
    const percentageChange = firstPeriodInterests > 0
        ? ((interestChange / firstPeriodInterests) * 100).toFixed(1)
        : "N/A";
    const isPositiveTrend = interestChange >= 0;
            ...
            <CardHeader className="flex flex-row items-start justify-between max-lg:!px-3">
                <div>
                    <CardTitle className="uppercase text-[#2F4A9D] font-medium tracking-widest text-base">
                        Package Interests Over Time
                    </CardTitle>
                    <CardDescription className="text-xs text-gray-500">
                        Showing package interest clicks for the {timeframeLabel}
                    </CardDescription>
                </div>
            ...
            <CardContent>
                {loading ? (
                    ...
                    >
                        ...
                        >
                            <defs>
                                <linearGradient id="fillInterests" x1="0" y1="0" x2="0" y2="1">
                                    <stop offset="5%" stopColor="var(--color-interests)" stopOpacity={0.8} />
                                    <stop offset="95%" stopColor="var(--color-interests)" stopOpacity={0.1} />
                                </linearGradient>
                            </defs>
                            ...
                            <Area
                                dataKey="interests"
                                type="natural"
                                fill="url(#fillInterests)"
                                fillOpacity={0.4}
                                stroke="var(--color-interests)"
                                strokeWidth={2}
                            />
                        </AreaChart>
                    </ChartContainer>
...
export default PackageInterestsOverTime
