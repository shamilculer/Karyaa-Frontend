...
/**
 * @desc Fetches packages for a specific vendor.
...
export const getVendorPackages = async (vendorId) => {
    ...
    try {
        const response = await apiFetch(`/packages/${vendorId}`);
        return { data: response.data || [] };
    } catch (error) {
        console.error(`Error fetching packages for vendor ${vendorId}:`, error);
        return { error: error.message || "Failed to fetch packages.", data: [] };
    }
};

/**
 * @desc Fetches reviews for a specific vendor.
 * @param {string} vendorId - The ID of the vendor.
...
export const getVendorReviews = async (vendorId) => {
    if (!vendorId) return { error: "Vendor ID is required." };
    try {
        const response = await apiFetch(`/vendors/${vendorId}/reviews`);
        return { reviews: response.data || [] };
    } catch (error) {
        console.error(`Error fetching reviews for vendor ${vendorId}:`, error);
        return { error: error.message || "Failed to fetch reviews.", reviews: [] };
    }
};
