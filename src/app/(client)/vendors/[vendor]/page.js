...
import { Heart, MapPin, Plus, Star, User } from "lucide-react";
...
import VendorSaveButton from "../../components/VendorSaveButton";
import VendorShareButton from "../../components/VendorShareButton";
...
const VendorPage = async ({ params }) => {
  ...
  const hasPackages = vendorData.packages > 0;


  return (
    <div className="min-h-screen">
      {/* ScrollHeaderWrapper (Assuming it includes VendorPopupHeader) */}
      <ScrollHeaderWrapper vendorData={{
      ...
      <div className="container flex flex-col lg:flex-row gap-6 lg:gap-12 !px-4 sm:!px-6 md:!px-10 relative">
        ...
        <div className="w-full flex items-start lg:items-center -mt-6 lg:mt-0">
          <div className="w-full pr-0 lg:pr-10 relative">
            <h1 className="!text-[32px] md:text-4xl leading-tight font-bold !capitalize">{vendorData.businessName}</h1>
            ...
            <div className="w-full flex flex-wrap items-center justify-between gap-x-7 gap-y-3 sm:gap-y-2 mt-4">
              ...
              <Link className="!font-medium font-heading flex items-center gap-1" href={`/compare?vendors=${vendorData.slug}`}><Plus /> Compare {vendorData.businessName}</Link>
            </div>
            ...
            <div className="absolute top-0 right-0 flex items-center gap-3 max-lg:-top-16 max-lg:right-2">
              ...
              {authResult.isAuthenticated && (
                <VendorSaveButton vendorId={vendorData._id} isInitialSaved={savedVendorIds.includes(vendorData._id || vendorData.id)} isVendorPage={true} />
              )}
              <VendorShareButton businessName={vendorData.businessName} slug={vendorData.slug} />
                        ...
                        {[5, 4, 3, 2, 1].map((star) => {
                          ...
                          return (
                            <div key={star} className="w-full flex items-center gap-3">
                              {/* Star Label */}
                              <div className="flex items-center">{star} <Star className="text-yellow-300 fill-yellow-300 w-3 h-3" /></div>
