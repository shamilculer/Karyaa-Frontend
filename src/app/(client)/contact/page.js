...
const ContactPage = async () => {
    ...
    if (contentResult?.success && contentResult?.data?.content) {
        try {
            ...
            pageContent = {
                ...
                bannerImage: parsedContent.bannerImage || pageContent.bannerImage,
    ...
    return (
        <div className="min-h-screen">
            <PageTitle
                placement="Contact"
                {...(pageContent.bannerImage && { imgUrl: pageContent.bannerImage })}
                {...(!pageContent.bannerImage && { imgUrl: "/banner-1.avif" })}
                ...
                <div className="w-full flex-center max-md:flex-col gap-20">
                    <div className="w-full md:w-1/2 space-y-6 md:space-y-10">
                        ...
                        <div className="space-y-8">
                            <Link href={`tel:${pageContent.primaryPhone}`} className="flex items-center gap-4">
                                <PhoneCall size={25} />
                                <span className="font-medium text-xl">{pageContent.primaryPhone}</span>
                            </Link>

                            <Link href={`mailto:${pageContent.mainEmail}`} className="flex items-center gap-4">
                                <Mail size={25} />
                                <span className="font-medium text-xl">{pageContent.mainEmail}</span>
                            </Link>

                            <span href="" className="flex items-center gap-4">
                                <MapPin size={25} />
