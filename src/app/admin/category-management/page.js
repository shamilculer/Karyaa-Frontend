...
const CategoriesContent = () => {
    ...
    const filteredCategories = useMemo(() => {
        ...
        return categories.filter(cat => {
            ...
            const matchesCategory = cat.name.toLowerCase().includes(lowerCaseQuery) ||
    ...
    return (
        <Card className="p-2 border-gray-200 shadow-none bg-white min-h-[500px] relative">
                ...
                <div className="px-2 mt-3">
                    ...
                    <ul className="list-disc list-inside text-sm text-gray-600 space-y-1 pl-4">
                        {category.subCategories?.slice(0, 3).map((sub) => (
                            <li key={sub._id} className="truncate">{sub.name}</li>
                        ))}
            ...
            <CardFooter className="!py-4 px-0 border-t border-t-gray-300 flex items-center justify-end gap-6 absolute w-[90%] bottom-0">
                <Button size="sm" className="bg-body text-primary border border-gray-300" asChild>
                    <Link href={`/admin/category-management/${category.slug}`} className="flex items-center gap-1">
