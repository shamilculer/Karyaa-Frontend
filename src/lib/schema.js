...
const objectIdString = z
  .string()
  .length(24, "ID must be a 24-character hexadecimal string.");
...
export const userSchema = z.object({
  ...
  mobileNumber: z.string().trim().regex(phoneRegex, {
    message:
...
export const Step2Schema = z
  .object({
    ...
    subCategories: z.array(objectIdString).optional().default([]),
    ...
    selectedBundle: objectIdString.min(1, "A Launch Bundle must be selected."),
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.mainCategory.length > 0 && data.subCategories.length === 0) {
      ctx.addIssue({
        ...
        path: ["subCategories"],
        message:
          "At least one Sub Category is required if a Main Category is selected.",
      });
    }
...
export const packageSchema = z.object({
  ...
  priceStartingFrom: z.preprocess(
    (a) => (a === "" ? 0 : a),
    z.coerce
      .number()
...
export const bundleFormSchema = z.object({
  ...
  durationValue: z
    .number()
    .min(1, { message: "Duration value must be at least 1." }),
  ...
  maxVendors: z
    .number()
    .min(1, "Max vendors must be at least 1, or leave blank/0 for unlimited.")
    .nullable()
    .optional(), // Added
